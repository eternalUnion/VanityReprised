...
public class RudeUpdater : Editor
{
    public const string RUDE_VERSION_PATH = "RudeVersion.json";
    ...
    [MenuItem("RUDE/Check for updates", priority = 1001)]
    ...
    {
        // Load online catalog
        UnityWebRequest catalogReq = new UnityWebRequest("https://raw.githubusercontent.com/eternalUnion/RudeUpdates/main/UpdateCatalog.json");
...

        {
            ...
            do
            {
                canceled = EditorUtility.DisplayCancelableProgressBar("Rude Updater", "Downloading catalog...", catalogHandler.progress);
                Task.Delay(100).Wait();
            }
        ...
        if (catalogReq.isNetworkError || catalogReq.isHttpError)
        {
            catalogReq.Dispose();
            if (!silent)
                EditorUtility.DisplayDialog("Rude Updater", "Failed to download update catalog", "Close");
            return;
        }

        RudeUpdateCatalog catalog;
        try
        {
            catalog = JsonConvert.DeserializeObject<RudeUpdateCatalog>(catalogReq.downloadHandler.text);
            catalogReq.Dispose();
        }
        catch (Exception ex)
        {
            ...
            if (!silent)
                EditorUtility.DisplayDialog("Rude Updater", "Failed to load update catalog", "Close");
            catalogReq.Dispose();
            return;
        }

        // Load local file
        RudeVersionInfo versionInfo;
        if (File.Exists(RUDE_VERSION_PATH))
        {
            try
            {
                versionInfo = JsonConvert.DeserializeObject<RudeVersionInfo>(File.ReadAllText(RUDE_VERSION_PATH));
            }
            catch (Exception e)
            {
                ...
                if (!EditorUtility.DisplayDialog("Rude Updater", $"Failed to load version info file at {RUDE_VERSION_PATH}. Would you like to reapply all patches?", "Yes", "Cancel"))
                    return;

                versionInfo = new RudeVersionInfo();
                ...
                File.WriteAllText(RUDE_VERSION_PATH, JsonConvert.SerializeObject(versionInfo, Formatting.Indented));
            }
        }
        else
        {
            Debug.Log($"Could not locate version file at {RUDE_VERSION_PATH}");
        ...
        {
            if (!silent)
                EditorUtility.DisplayDialog("Rude Updater", $"Warning: Current rude project is built using an older version of the game/vanity. Current branch is '{versionInfo.CurrentBranch}'. Latest branch is '{catalog.CurrentBranch}'.", "Ok");
        }
        ...
        try
        {
            foreach (RudeFileInfo fileInfo in catalog.Files)
            {
                ...
                {
                    if (!silent)
                        EditorUtility.DisplayDialog("Rude Updater", "Failed to download file", "Close");
                    fileReq.Dispose();
                    continue;
                }
        ...
        try
        {
            if (catalog.Updates.TryGetValue(versionInfo.CurrentBranch, out List<RudeUpdateInfo> patches))
            {
                foreach (RudeUpdateInfo patch in patches)
                {
...

                    {
                        ...
                        do
                        {
                            canceled = EditorUtility.DisplayCancelableProgressBar("Rude Updater", "Downloading patch...", patchHandler.progress);
                            Task.Delay(100).Wait();
                        }
                    ...
                    {
                        if (!silent)
                            EditorUtility.DisplayDialog("Rude Updater", $"Failed to download patch {patch.Version}", "Close");
                        patchRequest.Dispose();
                        if (File.Exists(patchFilePath))
                            File.Delete(patchFilePath);
                        return;
        ...
        finally
        {
            ...
            {
                if (!silent)
                    EditorUtility.DisplayDialog("Rude Updater", $"Rude is up to date", "Close");
            }
            else
            {
                EditorUtility.DisplayDialog("Rude Updater", $"{fileUpdateCount} {(fileUpdateCount == 1 ? "file" : "files")} updated. {patchCount} {(patchCount == 1 ? "patch" : "patches")} applied.", "Close");
            }
