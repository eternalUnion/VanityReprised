Shader "psx/unlit/transparent/unlit-scrolling" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                float4 tmp3;
                float4 tmp4;
                tmp0.x = _LocalOffset == 1.0;
                tmp0.yzw = max(_FlowDirection.xyz, float3(0.0, 0.0, 0.0));
                ...
                tmp2.xyz = tmp1.xyz - unity_ObjectToWorld._m03_m13_m23;
                tmp1.w = dot(tmp2.xyz, tmp0.xyz);
                tmp1.w = tmp1.w * _VertexNoiseScale;
                tmp2.x = _VertexNoiseSpeed * _Time.x;
                tmp1.w = tmp1.w * 0.1 + tmp2.x;
                tmp2.y = dot(tmp1.xyz, tmp0.xyz);
                tmp2.y = tmp2.y * _VertexNoiseScale;
                tmp2.x = tmp2.y * 0.1 + tmp2.x;
                tmp0.x = tmp0.x ? tmp1.w : tmp2.x;
                tmp2 = tex2Dgrad(_VertexNoise, tmp0.xx, 0, 0);
                tmp0.x = tmp2.x - 0.5;
                tmp0.x = tmp0.x * _VertexNoiseAmplitude;
                tmp0.xyz = tmp0.yzw * tmp0.xxx + tmp1.xyz;
                tmp1.xyz = tmp1.xyz - _MeshCenter.xyz;
                ...
                tmp0.w = tmp0.w * 0.2;
                tmp0.w = exp(tmp0.w);
                tmp0.w = tmp0.w * 0.1 + _VertexWarpScale;
                tmp0.w = tmp0.w * _VertexWarping;
                tmp2.w = max(_ScreenParams.y, _ScreenParams.x);
                tmp3.xy = _ScreenParams.xy / tmp2.ww;
                ...
                tmp0 = tmp4 + unity_MatrixVP._m03_m13_m23_m33;
                tmp4.xyz = tmp0.xyz / tmp0.www;
                tmp3.zw = tmp3.xy * tmp4.xy;
                tmp3.zw = floor(tmp3.zw);
                tmp3.zw = tmp3.zw + float2(0.5, 0.5);
                tmp4.xy = tmp3.zw / tmp3.xy;
                tmp3.xyz = tmp0.www * tmp4.xyz;
                tmp2.w = _VertexWarping != 0.0;
                o.position.xyz = tmp2.www ? tmp3.xyz : tmp0.xyz;
                o.position.w = tmp0.w;
                ...
                tmp0.w = rsqrt(tmp0.w);
                tmp4.xyz = tmp3.xyz * tmp0.www + -tmp3.xyz;
                tmp0.w = _TextureWarping + _TextureWarping;
                tmp0.w = min(tmp0.w, 1.0);
                o.texcoord2.xyz = tmp0.www * tmp4.xyz + tmp3.xyz;
                ...
                o.color1.w = saturate(tmp1.w / tmp0.x);
