Shader "psx/unlit/transparent/nocull-fresnel-fogfix" {
	Properties {
		_Color ("Color", Vector) = (1,1,1,1)
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_OpacScale ("Transparency Scalar", Range(0, 1)) = 1
		_FresnelCol ("Fresnel Color", Vector) = (0,0.25,1,1)
		_VertexWarpScale ("Vertex Warping Scalar", Range(0, 10)) = 1
		[Toggle] _ShouldForceOutlines ("Should Outline Objects Behind", Float) = 0
	}
	SubShader {
		LOD 200
		Tags { "LIGHTMODE" = "Vertex" "PASSFLAGS" = "OnlyDirectional" "QUEUE" = "Transparent" "RenderType" = "Transparent" }
		Pass {
			LOD 200
			Tags { "LIGHTMODE" = "Vertex" "PASSFLAGS" = "OnlyDirectional" "QUEUE" = "Transparent" "RenderType" = "Transparent" }
			...
			fout frag(v2f inp, float facing: VFACE)
			{
                ...
                tmp0.w = rsqrt(inp.color1.w);
                tmp0.w = 1.0 / tmp0.w;
                tmp1.xyz = tmp1.xyz * inp.color.xyz + -inp.color1.xyz;
                tmp1.xyz = tmp0.www * tmp1.xyz + inp.color1.xyz;
                tmp1.w = tmp1.y + tmp1.x;
                tmp1.w = tmp1.z + tmp1.w;
                tmp1.w = tmp1.w * inp.texcoord4.w;
                tmp1.w = tmp1.w * 0.3333333;
                tmp1.xyz = tmp1.www * _FresnelCol.xyz + tmp1.xyz;
                tmp0.z = saturate(inp.texcoord4.w * tmp0.z + tmp0.z);
                tmp0.z = tmp0.z * _OpacScale;
                tmp1.w = _HasSandBuff > 0.0;
                if (tmp1.w) {
                    ...
                    tmp1.w = 1.0 / tmp1.w;
                    tmp2.w = tmp1.w * 0.75 + 0.25;
                    tmp3.xyz = tmp2.xyz - tmp1.xyz;
                    tmp3.xyz = tmp2.www * tmp3.xyz + tmp1.xyz;
                    tmp2.xyz = tmp1.www * tmp2.xyz;
                    tmp1.xyz = tmp2.xyz * float3(0.75, 0.75, 0.75) + tmp3.xyz;
                }
                o.sv_target.w = tmp0.z * tmp0.w;
