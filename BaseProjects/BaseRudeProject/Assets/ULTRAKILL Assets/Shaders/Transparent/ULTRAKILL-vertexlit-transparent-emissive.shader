Shader "psx/vertexlit/transparent/transparent-emissive" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp, float facing: VFACE)
			{
                ...
                tmp0.w = _UseAlbedoAsEmissive == 0.0;
                tmp2.xyz = tmp0.www ? _EmissiveColor.xyz : tmp1.xyz;
                tmp2.xyz = tmp2.xyz * _EmissiveIntensity.xxx;
                tmp2.xyz = max(tmp2.xyz, float3(0.0, 0.0, 0.0));
                tmp3 = tex2D(_EmissiveTex, tmp0.xy);
                tmp0.w = _RGBEmissive == 0.0;
                tmp1.w = saturate(dot(tmp3.xyz, float3(0.3, 0.59, 0.11)));
