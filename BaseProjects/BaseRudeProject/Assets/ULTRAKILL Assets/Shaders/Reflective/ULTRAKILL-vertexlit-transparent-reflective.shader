	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp, float facing: VFACE)
			{
                ...
                tmp2 = texCUBE(_Cube, inp.texcoord2.xyz);
                tmp2.xyz = tmp2.xyz * _ReflectionStrength.xxx;
                tmp0.w = dot(tmp2.xyz, float3(0.333, 0.333, 0.333));
                tmp1.w = 1.0 - _ReflectionStrength;
                tmp1.xyz = saturate(tmp1.xyz * tmp1.www + tmp2.xyz);
                tmp1.w = saturate(tmp0.z * _OpacScale + tmp0.w);
