Shader "psx/vertexlit/fillblood" {
	Properties {
		...
		_MeshScale ("Mesh Size", Vector) = (0,0,0,0)
		_MeshCenter ("Mesh Center", Vector) = (0,0,0,0)
		_BloodNoise ("BloodNoise", 2D) = "white" {}
	}
	SubShader {
		LOD 200
		...
		Pass {
			...
			fout frag(v2f inp, float facing: VFACE)
			{
                ...
                tmp0.zw = inp.texcoord8.xy - float2(0.5, 0.5);
                tmp1.w = min(abs(tmp0.w), abs(tmp0.z));
                tmp2.x = max(abs(tmp0.w), abs(tmp0.z));
                tmp2.x = 1.0 / tmp2.x;
                ...
                tmp2.x = tmp0.z * 0.1591549 + 0.5;
                tmp0.z = inp.texcoord8.z * _MeshScale.z;
                tmp2.y = tmp0.z * 0.5;
                tmp2 = tex2D(_MainTex, tmp2.xy);
                tmp3 = tex2D(_MainTex, inp.texcoord8.xy);
                tmp0.z = saturate(abs(inp.texcoord3.y) * 2.0 + -0.5);
                tmp0.w = tmp3.x - tmp2.x;
                tmp0.z = saturate(tmp0.z * tmp0.w + tmp2.x);
                tmp0.z = tmp0.z * 2.0 + -1.0;
                tmp0.w = _FillAmount > 0.0;
                tmp0.z = -tmp0.z * 0.05 + inp.texcoord8.z;
                tmp0.z = tmp0.z < _FillAmount;
                tmp2.xyz = tmp0.zzz ? float3(1.0, 0.0, 0.0) : float3(1.0, 1.0, 1.0);
