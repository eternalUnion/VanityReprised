Shader "psx/vertexlit/vertexlit-customcolors-emissive" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp, float facing: VFACE)
			{
                ...
                tmp1 = tex2D(_IDTex, tmp0.xy);
                tmp0.z = tmp1.x * _CustomColor1.w;
                tmp2.xyz = tmp1.xyz * float3(0.8, 0.8, 0.8);
                tmp3.xyz = tmp1.www * _CustomColor1.xyz + -tmp1.www;
                tmp3.xyz = tmp2.xxx * tmp3.xyz + tmp1.www;
                tmp0.w = tmp1.y * _CustomColor2.w;
                tmp4.xyz = tmp1.www * _CustomColor2.xyz + -tmp3.xyz;
                tmp2.xyw = tmp2.yyy * tmp4.xyz + tmp3.xyz;
                tmp1.y = tmp1.z * _CustomColor3.w;
                tmp3.xyz = tmp1.www * _CustomColor3.xyz + -tmp2.xyw;
                tmp2.xyz = tmp2.zzz * tmp3.xyz + tmp2.xyw;
                tmp3 = texCUBE(_Cube, inp.texcoord2.xyz);
                tmp3.xyz = tmp2.xyz * tmp3.xyz;
                tmp1.z = tmp1.w * 2.0 + -1.0;
                tmp2.w = max(_HighlightFalloff, 0.5);
                tmp1.z = log(abs(tmp1.z));
                tmp1.z = tmp1.z * tmp2.w;
                tmp2.w = exp(tmp1.z);
                tmp3.w = saturate(_HighlightStrength);
                tmp2.w = tmp2.w * tmp3.w;
                tmp4.xyz = tmp1.www - tmp2.xyz;
                tmp2.xyz = tmp2.www * tmp4.xyz + tmp2.xyz;
                tmp1.x = -tmp1.x * _CustomColor1.w + 1.0;
                tmp0.z = tmp0.w * tmp1.x + tmp0.z;
                tmp0.w = 1.0 - tmp0.z;
                tmp0.z = tmp1.y * tmp0.w + tmp0.z;
                tmp0.w = tmp1.z * 0.1;
                tmp0.w = exp(tmp0.w);
                tmp0.z = tmp0.z * tmp0.w;
                tmp1.xyz = tmp3.xyz * tmp0.zzz + tmp2.xyz;
                ...
                tmp2.xyz = tmp0.zzz ? _EmissiveColor.xyz : tmp1.xyz;
                tmp2.xyz = tmp2.xyz * _EmissiveIntensity.xxx;
                tmp2.xyz = max(tmp2.xyz, float3(0.0, 0.0, 0.0));
                tmp3 = tex2D(_EmissiveTex, tmp0.xy);
                tmp0.z = _RGBEmissive == 0.0;
                tmp0.w = saturate(dot(tmp3.xyz, float3(0.3, 0.59, 0.11)));
                tmp3.xyz = tmp0.zzz ? tmp0.www : tmp3.xyz;
                tmp1.xyz = tmp2.xyz * tmp3.xyz + tmp1.xyz;
                tmp2.xyz = tmp2.xyz - tmp1.xyz;
                tmp2.xyz = tmp2.xyz * tmp3.xyz;
                tmp1.xyz = _EmissiveReplaces.xxx * tmp2.xyz + tmp1.xyz;
