Shader "psx/vertexlit/vertexlit-customcolors-railgun" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp2.xyz = tmp1.www * _CustomColor1.xyz + -tmp1.www;
                tmp3.xyz = tmp1.xyz * float3(0.8, 0.8, 0.8);
                tmp2.xyz = tmp3.xxx * tmp2.xyz + tmp1.www;
                tmp4.xyz = tmp1.www * _CustomColor2.xyz + -tmp2.xyz;
                tmp2.xyz = tmp3.yyy * tmp4.xyz + tmp2.xyz;
                tmp3.xyw = tmp1.www * _CustomColor3.xyz + -tmp2.xyz;
                tmp2.xyz = tmp3.zzz * tmp3.xyw + tmp2.xyz;
                tmp3.xyz = tmp1.www - tmp2.xyz;
                tmp0.z = saturate(_HighlightStrength);
                tmp0.w = max(_HighlightFalloff, 0.5);
                tmp1.w = tmp1.w * 2.0 + -1.0;
                tmp1.w = log(abs(tmp1.w));
                ...
                tmp1.y = tmp1.y * _CustomColor2.w;
                tmp1.x = -tmp1.x * _CustomColor1.w + 1.0;
                tmp1.z = tmp1.z * _CustomColor3.w;
                tmp0.z = tmp1.y * tmp1.x + tmp0.z;
                tmp1.x = 1.0 - tmp0.z;
                tmp0.z = tmp1.z * tmp1.x + tmp0.z;
                tmp0.z = tmp0.z * tmp0.w;
                tmp1 = texCUBE(_Cube, inp.texcoord2.xyz);
                tmp1.xyz = tmp2.xyz * tmp1.xyz;
                tmp1.xyz = tmp1.xyz * tmp0.zzz + tmp3.xyz;
                tmp1.xyz = tmp1.xyz * inp.color.xyz;
                tmp0.z = 1.0 - inp.color1.w;
                tmp2.xyz = tmp0.zzz * inp.color1.xyz;
                tmp1.xyz = tmp1.xyz * inp.color1.www + tmp2.xyz;
                tmp0.z = tmp0.x - 4.0;
                tmp2 = tex2D(_MainTex, tmp0.xy);
                tmp0.x = tmp2.w * inp.color.w;
                tmp0.y = floor(tmp0.z);
                tmp0.y = saturate(tmp0.y + _EmissivePosition);
                tmp0.y = tmp0.y * _EmissiveIntensity;
                tmp0.x = saturate(tmp0.x * tmp0.y);
                o.sv_target.xyz = _EmissiveColor.xyz * tmp0.xxx + tmp1.xyz;
                o.sv_target.w = 1.0;
                return o;
			}
			ENDCG
