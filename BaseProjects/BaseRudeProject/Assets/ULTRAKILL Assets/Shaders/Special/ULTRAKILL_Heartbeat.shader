	...
	SubShader {
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                tmp0.w = v.texcoord.y * _VertexNoiseScale;
                tmp0.w = tmp0.w * 0.1;
                tmp0.w = frac(tmp0.w);
                tmp1.w = _UseVCols == 1.0;
                tmp2.w = v.color.x * 2.0 + -1.0;
                tmp3.w = _VertexNoiseSpeed * _Time.y;
                tmp4.w = sin(tmp3.w);
                tmp2.w = tmp2.w * tmp4.w;
                tmp2.w = tmp2.w * 0.5 + 0.5;
                tmp0.w = tmp0.w * _VertexNoiseScale + tmp3.w;
                tmp0.w = frac(tmp0.w);
                tmp0.w = tmp0.w * 2.0 + -1.0;
                tmp0.w = tmp1.w ? tmp2.w : abs(tmp0.w);
                tmp5.xyz = tmp0.www * tmp3.xyz;
                tmp5.xyz = tmp5.xyz * _VertexNoiseAmplitude.xxx;
                tmp4.xyz = tmp5.xyz * float3(5.0, 5.0, 5.0) + tmp4.xyz;
                ...
                tmp6.xyz = tmp7.yyy * unity_MatrixV._m01_m11_m21;
                tmp6.xyz = unity_MatrixV._m00_m10_m20 * tmp7.xxx + tmp6.xyz;
                tmp6.xyz = unity_MatrixV._m02_m12_m22 * tmp7.zzz + tmp6.xyz;
                tmp6.xyz = unity_MatrixV._m03_m13_m23 * tmp7.www + tmp6.xyz;
                ...
                for (int i = tmp1.w; i < 8; i += 1) {
                    tmp7.xyz = -tmp6.xyz * unity_LightPosition[i].www + unity_LightPosition[i].xyz;
