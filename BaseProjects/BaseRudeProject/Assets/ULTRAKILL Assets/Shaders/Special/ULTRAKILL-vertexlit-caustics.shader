	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp, float facing: VFACE)
			{
                ...
                tmp2.xyz = inp.color1.www * tmp1.xyz + inp.color1.xyz;
                ...
                tmp0.zw = inp.texcoord1.xz * _Scale.xx;
                tmp1.x = _Speed * _Time.y;
                tmp1.y = tmp1.x * 0.1;
                tmp1.yz = tmp0.zw * float2(0.02, 0.02) + tmp1.yy;
                tmp3 = tmp0.zwzw * float4(0.02, 0.02, 0.02, 0.02) + float4(0.36, -0.19, 0.62, 0.62);
                tmp3.xy = tmp1.xx * float2(-0.1, 0.0) + tmp3.xy;
                tmp3.zw = tmp1.xx * float2(-0.1, 0.1) + tmp3.zw;
                tmp0.zw = tmp0.zw * float2(0.02, 0.02) + float2(-0.61, 0.62);
                tmp0.zw = tmp1.xx * float2(0.0, 0.1) + tmp0.zw;
                tmp1 = tex2D(_MainTex, tmp1.yz);
                ...
                tmp3 = tmp3 * tmp3;
                tmp1 = tmp1 * tmp3;
                tmp0.z = saturate(inp.texcoord3.y * _Falloff);
                tmp0.z = tmp0.z * tmp0.z;
                tmp1 = tmp0.zzzz * tmp1;
                tmp1 = tmp1 * _CausticColor;
