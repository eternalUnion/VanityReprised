	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp, float facing: VFACE)
			{
                ...
                tmp0.z = max(_GradientScale, 0.01);
                tmp0.zw = tmp0.zz * float2(100.0, 628.3185);
                tmp3.xyz = inp.texcoord1.xyz - _WorldOffset;
                tmp1.w = dot(tmp3.xyz, tmp3.xyz);
                tmp1.w = sqrt(tmp1.w);
                tmp0.z = tmp1.w / tmp0.z;
                tmp2.w = _GradientSpeed * _Time.x;
                tmp0.z = -_Time.x * _GradientSpeed + tmp0.z;
                tmp0.z = frac(tmp0.z);
                tmp0.z = tmp0.z * 2.0 + -1.0;
                tmp0.z = log(abs(tmp0.z));
                tmp0.z = tmp0.z * _GradientFalloff;
                tmp0.z = exp(tmp0.z);
                tmp3.x = floor(_RainbowDensity);
                tmp0.w = tmp0.w / tmp3.x;
                tmp0.w = tmp1.w / tmp0.w;
                tmp0.w = -tmp2.w * 0.3183101 + tmp0.w;
                tmp0.w = tmp0.w + 0.5;
                tmp0.w = frac(tmp0.w);
                tmp0.w = tmp0.w * 6.283185;
                tmp3.x = sin(tmp0.w);
                tmp4.x = cos(tmp0.w);
                tmp3.yzw = tmp2.zxy * float3(0.57735, 0.57735, 0.57735);
                tmp3.yzw = tmp2.zxy * float3(0.57735, 0.57735, 0.57735) + -tmp3.wyz;
                tmp3.xyz = tmp3.xxx * tmp3.yzw;
                tmp3.xyz = tmp2.xyz * tmp4.xxx + tmp3.xyz;
                tmp0.w = dot(float3(0.57735, 0.57735, 0.57735), tmp2.xyz);
                tmp0.w = tmp0.w * 0.57735;
                tmp1.w = 1.0 - tmp4.x;
                tmp3.xyz = tmp0.www * tmp1.www + tmp3.xyz;
                tmp0.w = _PCGamerMode == 0.0;
