Shader "psx/vertexlit/vertexlit-bloodabsorber" {
	Properties {
		_Color ("Color", Vector) = (1,1,1,1)
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_BloodTex ("BloodTex", 2D) = "black" {}
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp, float facing: VFACE)
			{
                ...
                tmp0.zw = inp.texcoord6.xy * float2(1.0, -1.0);
                ...
                tmp1.w = tmp1.w + 0.5;
                tmp1.w = floor(tmp1.w);
                tmp2.xyz = tmp1.www * float3(0.0, -0.9, -0.9) + float3(1.0, 1.0, 1.0);
                tmp1.xyz = tmp1.xyz * tmp2.xyz;
                tmp1.w = _IsWashing == 1.0;
                if (tmp1.w) {
                    tmp2 = tex2Dlod(_MainTex, float4(tmp0.zw, 0, 0.0));
                    tmp0.z = rsqrt(tmp2.x);
                    tmp0.z = 1.0 / tmp0.z;
                ...
                tmp2.xyz = tmp0.zzz * float3(-0.5, -0.4, -0.35) + float3(1.0, 1.0, 1.0);
                ...
                if (tmp0.z) {
                    ...
                    tmp4.xy = tmp0.yz * -tmp1.ww + -inp.texcoord4.xy;
                    tmp0.y = dot(tmp0.xyz, inp.texcoord4.xyz);
                    tmp0.y = 1.0 - abs(tmp0.y);
                    tmp0.zw = tmp4.xy * float2(0.05, 0.05);
                    tmp4 = tex2D(_MainTex, tmp0.zw);
                    tmp0.z = tmp4.x * 0.01 + tmp0.y;
                    tmp0.z = max(tmp0.z, 0.01);
                    tmp0.z = min(tmp0.z, 0.99);
                    tmp5.x = tmp0.z * tmp0.z;
                    tmp0.z = tmp0.x * 2.0;
                    tmp5.y = tmp4.x * 5.0 + tmp0.z;
                    tmp4 = tex2D(_MainTex, tmp5.xy);
                    tmp0.y = log(tmp0.y);
                    tmp0.y = tmp0.y * 0.75;
                    tmp0.y = exp(tmp0.y);
                    ...
                    tmp1.xyz = tmp1.xyz * tmp2.xyz + -tmp0.yzw;
