Shader "psx/unlit/alphatest/emissive" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			float _EmissiveReplaces;
			float _UseAlbedoAsEmissive;
			...
			fout frag(v2f inp, float facing: VFACE)
			{
                ...
                tmp3.xy = ddx(tmp0.xy);
                tmp3.zw = ddy(tmp0.xy);
                tmp3.xy = abs(tmp3.zw) + abs(tmp3.xy);
                tmp3.xy = max(tmp3.xy, float2(0.0001, 0.0001));
                tmp0.zw = tmp0.zz / tmp3.xy;
                ...
                tmp0.z = _UseAlbedoAsEmissive == 0.0;
                tmp3.xyz = tmp0.zzz ? _EmissiveColor.xyz : tmp1.xyz;
                tmp3.xyz = tmp3.xyz * _EmissiveIntensity.xxx;
                tmp3.xyz = max(tmp3.xyz, float3(0.0, 0.0, 0.0));
                tmp4 = tex2D(_EmissiveTex, tmp0.xy);
                tmp0.z = _RGBEmissive == 0.0;
                tmp0.w = saturate(dot(tmp4.xyz, float3(0.3, 0.59, 0.11)));
                tmp4.xyz = tmp0.zzz ? tmp0.www : tmp4.xyz;
                tmp1.xyz = tmp3.xyz * tmp4.xyz + tmp1.xyz;
                tmp3.xyz = tmp3.xyz - tmp1.xyz;
                tmp3.xyz = tmp3.xyz * tmp4.xyz;
                tmp2.xyz = _EmissiveReplaces.xxx * tmp3.xyz + tmp1.xyz;
                tmp0.z = _HasSandBuff > 0.0;
                if (tmp0.z) {
                    tmp0.z = dot(inp.texcoord3.xyz, inp.texcoord3.xyz);
                    tmp0.z = rsqrt(tmp0.z);
                    tmp1.xyz = tmp0.zzz * inp.texcoord3.xyz;
                    tmp3 = _Time * float4(0.1, 0.5, 0.5, 0.1) + inp.texcoord1.xyyz;
                    tmp0.zw = max(abs(tmp1.zy), abs(tmp1.xx));
                    tmp0.zw = tmp0.zw < abs(tmp1.yz);
                    tmp1.xyz = tmp0.zzz ? float3(0.0, 1.0, 0.0) : float3(1.0, 0.0, 0.0);
                    tmp1.xyz = tmp0.www ? float3(0.0, 0.0, 1.0) : tmp1.xyz;
                    tmp4 = tmp1.zzxx * tmp3;
                    tmp0.zw = tmp4.zw + tmp4.xy;
                    tmp0.zw = tmp3.xw * tmp1.yy + tmp0.zw;
                    ...
                    tmp3.xyz = tmp0.www * tmp3.xyz + tmp2.xyz;
                    tmp1.xyz = tmp0.zzz * tmp1.xyz;
                    tmp2.xyz = tmp1.xyz * float3(0.75, 0.75, 0.75) + tmp3.xyz;
                }
