...
{
	internal partial class UnityPackageExporter
	{
		...
		public void Export(GameBundle fileCollection, LibraryConfiguration options)
		{
			...
			UnityPackageAssetContainer container = new UnityPackageAssetContainer(this, options, fileCollection.FetchAssets(), collections);
			...
			foreach (IUnityObjectBase asset in fileCollection.FetchAssets())
			{
				if (!queued.Contains(asset))
				{
					...
					if (collection is ExportCollection exportCollection)
					{
						...
						else if (GameData.ObjectsToMerge.ContainsKey(asset))
						{
							exportCollection._exportable = false;
						}
